...
export function ProfileView({ user, tasks, onLogout, isSuperAdmin, db, appId }) {
  ...
  const activarNotificaciones = async () => {
    ...
    if (permission === 'granted') {
        try { new Notification(getCachedAppConfig().institutionShortName || getCachedAppConfig().institutionName, { body: "Notificaciones activadas.", icon: '/icon-192.png' }); } catch (e) {}
        try {
